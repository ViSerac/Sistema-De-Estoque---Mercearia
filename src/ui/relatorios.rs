                ...
                .body(|mut body| {
                    ...
                    for p in &produtos {
                        body.row(26.0, |mut row| {
                            row.col(|ui| { ui.colored_label(Cores::LARANJA, &p.nome); });
                            row.col(|ui| { ui.label(&p.categoria_nome); });
                            row.col(|ui| { ui.colored_label(Cores::VERMELHO, p.quantidade_atual.to_string()); });
                            row.col(|ui| { ui.label(p.estoque_minimo.to_string()); });
                            row.col(|ui| {
